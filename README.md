...
Проект покрыт **29 автоматическими модульными тестами** на фреймворке **DUnitX** со **100% успешным прохождением**.
...
| `JpegUtils.pas` | 5 | Сжатие фото, ресайз до 1920px, сохранение пропорций, качество 85% |
...
- [x] ~~Подключение теста `TestJpegUtils.pas` к `TestRunner.dpr`~~ ✅ **Выполнено**
